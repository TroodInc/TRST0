...
async function runBlock(opts) {
    ...
    if (this._hardforkByBlockNumber ||
        (0, ethereumjs_util_1.isTruthy)(this._hardforkByTTD) ||
        (0, ethereumjs_util_1.isTruthy)(opts.hardforkByTTD)) {
    ...
    try {
        ...
        if (this.DEBUG) {
            debug(`Received block results gasUsed=${result.gasUsed} bloom=${(0, ethereumjs_util_1.short)(result.bloom.bitvector)} (${result.bloom.bitvector.length} bytes) receiptRoot=${result.receiptRoot.toString('hex')} receipts=${result.receipts.length} txResults=${result.results.length}`);
        }
    }
    catch (err) {
        await state.revert();
